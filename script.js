...
async function addGiocatore() {
  ...
  if (!nome) { showToast("Inserisci il nome del giocatore"); return; }
  if (listaAsta.length >= MAX_ASTA) { showToast("Slot asta esauriti (max 11)"); return; }
...
async function saveEdit() {
  ...
  if (!nome) { showToast("Nome obbligatorio"); return; }
